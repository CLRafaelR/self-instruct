...
def main(args):
    ...
    print(
        f"Tokens: {total_tokens} Time: {run_time:.2f} Tokens per second: {total_tokens/run_time:.2f}"
    )
    print()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        ...
        description="End-to-end token generation loop example for gen-ai",
    )
    parser.add_argument(
        "-m",
        "--model",
    ...
    parser.add_argument(
        "-p", "--top_p", type=float, help="Top p probability to sample with"
    )
    parser.add_argument("-k", "--top_k", type=int, help="Top k tokens to sample from")
    parser.add_argument(
        "-t", "--temperature", type=float, help="Temperature to sample with"
    )
    parser.add_argument(
        "-r",
        "--repetition_penalty",
